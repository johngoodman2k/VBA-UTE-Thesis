				...
				<Swiper
					...
					{data.map(item => {
						return (
							<SwiperSlide>
								<div className='w-full h-full  relative'>
									<img src={item.img} className='w-full h-full object-content ' alt=''></img>
								</div>
								<div
									slot='container-end'
									className={`w-3/12 h-2/6 mr-5 mb-5 z-10 right-0 bottom-0 absolute ${cx('Slider')}`}>
									<div className='w-full h-full  relative'>
										<img src={item.img} className='w-full h-full object-content ' alt=''></img>
									</div>
									<div
										slot='container-end'
										className={`bg-black w-3/12 h-full z-20 right-0 bottom-0 absolute ${cx('Slider__btnNext')}`}></div>
								</div>
							</SwiperSlide>
						);
					})}
